...
export default function ProductQuickView({ product, onClose }: ProductQuickViewProps) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#FAF8F5]/90 p-4 backdrop-blur-md">
        ...
        <motion.div
          ...
          className="relative max-w-3xl w-full border border-[#D4AF37]/20 bg-[#F0EDE8] p-6 md:p-10 shadow-2xl z-10 overflow-hidden"
        >
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 text-[#4A4A4A] hover:text-amber-400 transition-colors cursor-pointer"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="relative aspect-square overflow-hidden border border-[#D4AF37]/10 bg-[#F0EDE8] flex items-center justify-center">
              <img
                src={product.image}
                alt={product.name}
              ...
              <div className="absolute bottom-3 left-3 z-20 flex items-center space-x-1 border border-[#D4AF37]/45 bg-white/90 px-2 py-0.5 rounded-sm">
                ...
                <span className="text-[8px] font-mono tracking-widest text-[#1A1A1A] uppercase">BIS CERTIFIED 916</span>
              </div>
            </div>
            ...
            <div className="flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                ...
                <p className="font-serif italic text-[#4A4A4A] text-xs leading-relaxed">
                  {product.description}
                </p>
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-b border-[#D4AF37]/10 py-4 mt-2">
                  <div className="space-y-0.5">
                    <span className="text-[9px] uppercase tracking-wider text-[#4A4A4A] font-sans block">Metal Purity</span>
                    <span className="font-serif text-xs font-semibold text-[#1A1A1A]">{product.purity}</span>
                  </div>
                  {product.weight && (
                    <div className="space-y-0.5">
                      <span className="text-[9px] uppercase tracking-wider text-[#4A4A4A] font-sans block">Net Weight</span>
                      <span className="font-serif text-xs font-semibold text-[#1A1A1A]">{product.weight}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="pt-2">
                  <span className="text-[9px] uppercase tracking-wider text-[#4A4A4A] font-sans block">Estimated Evaluation</span>
                  ...
                  <span className="text-[8px] font-mono text-[#4A4A4A] mt-1 block">
              ...
              <div className="space-y-3.5">
                ...
                <button
                  onClick={onClose}
                  className="w-full border border-[#D4AF37]/10 text-[#1A1A1A] hover:text-[#1A1A1A] py-2.5 px-4 text-xs font-semibold uppercase tracking-widest transition-colors cursor-pointer"
                >
