      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="columns-1 gap-6 sm:columns-2 lg:columns-3">
          {GALLERY_ITEMS.map((item) => (
            <div
              key={item.id}
              className="break-inside-avoid mb-6 relative group overflow-hidden border border-[#D4AF37]/10 bg-[#F0EDE8] p-2.5 transition-all duration-500 hover:border-[#D4AF37]/30 shadow-lg"
            >
              <div className="relative overflow-hidden rounded-sm bg-zinc-900">
                ...
                <div className="absolute inset-0 bg-[#1A1A1A]/45 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-6 backdrop-blur-[1.5px]">
                  {/* Top-Right action to zoom */}
                  <div className="flex justify-end">
                    <button
                      onClick={() => handleOpenLightbox(item.image)}
                      onMouseEnter={playGlowChime}
                      className="h-10 w-10 flex items-center justify-center border border-[#D4AF37]/30 bg-[#F0EDE8] text-[#1A1A1A] hover:bg-[#D4AF37] hover:text-black hover:border-[#D4AF37] transition-all duration-300 cursor-pointer"
                  ...
                  <div className="space-y-3.5">
                    ...
                    <h3 className="font-serif text-base text-[#1A1A1A] tracking-wide leading-snug">
                      {item.title}
                    </h3>
                    <div className="pt-2 border-t border-zinc-805 flex items-center justify-between">
                      <button
                        onClick={() => handleShareOnWhatsApp(item.title)}
                        className="text-[10px] uppercase tracking-widest text-[#D4AF37] hover:text-[#1A1A1A] transition-colors duration-200 flex items-center space-x-1 font-semibold cursor-pointer"
                      >
                        <MessageSquare className="h-3 w-3 mr-1" /> WhatsApp Inquiry
                      </button>
                      <span className="text-[10px] text-[#4A4A4A] font-mono">REEL_#{item.id.replace('g-', '0')}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {selectedImage && (
            ...
            >
              <div className="absolute top-6 right-6 flex items-center space-x-4">
                <span className="text-[10px] tracking-widest uppercase text-[#4A4A4A] font-mono">Press Esc to close</span>
                <button
              ...
              >
                <img
                  ...
                  referrerPolicy="no-referrer"
                />
                <div className="mt-3.5 text-center text-xs tracking-widest uppercase text-[#4A4A4A] font-mono">
